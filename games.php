                ...
                <ul data-role="listview" data-inset="true" data-split-icon="edit">
                    ...
                    <?php for ($i = 0; $i <= count($games_list) - 1; $i++) {
                        ...
                        <li>
                            ...
                            <?php } ?>
                                 <span <?php  if($expired) {echo 'class="expired"';} ?>><?php echo date_to_php($games_list[$i]['game_name']) ?></span>
                            </a>
                            <?php if ($_SESSION['player_access'] == 'admin') { ?><a href="winners_update.php?game_id=<?php echo $games_list[$i]['game_id']; ?>&force_refresh=<?php echo mt_rand(1, 1000); ?>" title="Winners"></a><?php } ?>
                        </li>
                    <?php } ?>
                </ul>
            </div>
