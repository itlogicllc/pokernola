<a href="invitation_send.php?player_id=<?php echo $player_logged_in_id ?>" data-role="button" data-icon="mail" data-iconpos="notext">Invite Player</a>
<a href="logout.php?player_id=<?php echo $player_logged_in_id ?>" class="ui-btn-right" data-role="button" data-icon="power" data-iconpos="notext" data-transition="fade">Log Out</a>
<nav data-role="navbar">
	<ul>
		<li><a href="home.php" class="ui-btn-active ui-state-persist" data-transition="flip" data-icon="home">Home</a></li>
		<li><a href="players.php" data-transition="flip" data-icon="user">Players</a></li>
		<li><a href="games.php" data-transition="flip" data-icon="grid">Games</a></li>
		<li><a href="tools.php" data-transition="flip" data-icon="gear">Tools</a></li>
	</ul>
</nav>
